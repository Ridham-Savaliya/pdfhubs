...
function formatContent(content: string): string {
  ...
  // First, wrap list items in specific tags to avoid paragraph wrapping
  formatted = formatted
    // Checkmark lists
  ...
  // We use a temporary placeholder to avoid paragraph wrapping issues
  ...
  formatted = formatted.replace(/^\|(.+)\|$/gm, (match) => {
    ...
    // Determine if it looks like a header (simple heuristic: first row or caps)
    // Actually, properly parsing markdown tables requires lookahead/lookbehind which is hard with simple regex.
    // We'll treat the first row found in a block as header if we could, but for now let's just make it a row.
    // Better strategy: replace matches individually, then group them.

    // Check if the cell content starts with bold or is likely a header
    const isHeader = cells[0].trim().startsWith('**') || cells[0].trim().match(/^[A-Z\s]+$/);
    const tag = 'td'; // Default to td, styling will handle headers if needed or we assume simplified markdown tables
  ...
  formatted = chunks.map(chunk => {
    // If chunk matches an HTML tag we just created, leave it alone
