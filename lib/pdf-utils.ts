...
// Aggressive compression that actually reduces file size
export async function compressPDF(
  ...
  // Ultra-aggressive quality settings for maximum speed and compression
  const settings = {
    low: { scale: 0.6, imageQuality: 0.4 },      // Tiny files, fast
    medium: { scale: 0.75, imageQuality: 0.6 },  // Balanced
    high: { scale: 1.0, imageQuality: 0.75 },    // Better quality
  }[quality];

  const newPdf = await PDFDocument.create();

  // Process pages in controlled batches to avoid browser hanging
  const concurrency = 5; // Process 5 pages at a time
  const results: { jpegBytes: ArrayBuffer; width: number; height: number }[] = [];
  ...
  for (let i = 0; i < numPages; i += concurrency) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (pageIndex) => {
        ...
        return new Promise<{ jpegBytes: ArrayBuffer; width: number; height: number }>((resolve) => {
          canvas.toBlob(
            async (blob) => {
              const jpegBytes = await blob!.arrayBuffer();
              resolve({
                jpegBytes,
                width: viewport.width / settings.scale,
                height: viewport.height / settings.scale
              });
            },
            "image/jpeg",
    ...
    onProgress?.(10 + ((results.length / numPages) * 70));
  }

  onProgress?.(80);

  // Add all processed pages to new PDF
  for (const result of results) {
    const image = await newPdf.embedJpg(result.jpegBytes);
    const pdfPage = newPdf.addPage([result.width, result.height]);
